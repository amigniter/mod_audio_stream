...
def test_resample_8k_to_16k_length_scales() -> None:
    ...
    r = Resampler(in_rate=8000, out_rate=16000, channels=1)
    pcm16 = r.process(pcm8)
...
def test_stateful_resample_matches_whole() -> None:
    ...
    whole = Resampler(in_rate=8000, out_rate=16000).process(pcm8)

    r = Resampler(in_rate=8000, out_rate=16000)
    a = r.process(pcm8[: len(pcm8) // 2])
    b = r.process(pcm8[len(pcm8) // 2 :])
    chunked = a + b
...
def test_resample_8k_to_24k_length_scales() -> None:
    ...
    pcm8 = _tone_pcm16_mono(8000, 440.0, 200)
    r = Resampler(in_rate=8000, out_rate=24000, channels=1)
